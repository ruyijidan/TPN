...
def evaluate_results(results, filename_gt, test_dataset_size, num_classes,
    ...
    json_file['Num of empty videos'] = str(count_empty)
    json_file['Num of corrupted videos'] = str(count_corrupted)
    json_file['Max num of clips in a video'] = str(max_clips)
    json_file['Min num of clips in a video'] = str(min_clips)
    json_file['Clip1 accuracy'] = ': {:.2f} percent ({}/{})'.format(
        100. * clip1_accuracy / clip1_count, clip1_accuracy, clip1_count)
    json_file['Clip accuracy'] =': {:.2f} percent ({}/{})'.format(
        100. * clip_accuracy / len(results), clip_accuracy, len(results))
    json_file['top-1 accuracy'] = str(accuracy * 100)
    json_file['top-5 accuracy'] = str(accuracy_top5 * 100)

    print(json_file)
    with open(dir + '/eval.json', 'w') as f:
        json.dump(json_file, f, indent=4, sort_keys=False)

    return


def evaluate_infer_results(results, num_classes, num_test_clips, labels_list, dir):
    probs = {}
    counts = {}
    for entry in results:
        vid = entry[0]
    ...
    for vid in probs.keys():
        ...
        for ind in topk_inds:
            ...
            one_flie['class' + '_top:' + str(ii)] = str(labels_list[ind])
            one_flie['probability' + '_top:' + str(ii)] = str(pred[ind])

            json_file['video' + ':' + str(vid)] = one_flie

    print(json_file)
    with open(dir + '/infer.json', 'w') as f:
        json.dump(json_file, f, indent=4, sort_keys=False)
