...
with fluid.dygraph.guard():
    ...
    for torch_key in torch_weight.keys():
        # if torch_key.find('num_batches_tracked')>0:
        #     continue
        #     #print(torch_key,'-----',torch_weight[torch_key])
        for paddle_key in model_weight.keys():

            if torch_key == paddle_key:
                if torch_key.find('fc') > 0:
                    new_fan_weight_dict[paddle_key] = torch_weight[torch_key].detach().numpy().T
                    print('torch fc:', torch_key, ':::::::', paddle_key)
                else:
                    # if torch_key in ['necks.aux_head.convs.conv.weight', 'necks.aux_head.convs.bn.weight',
                    #                  'necks.aux_head.convs.bn.bias', 'necks.aux_head.convs.bn._mean',
                    #                  'necks.aux_head.convs.bn._variance']:
                    #     print('None',torch_key)
                    new_fan_weight_dict[paddle_key] = torch_weight[torch_key].detach().numpy()
                    print('torch:', torch_key, ':::::::', paddle_key)
            elif torch_key.find('running_mean') and paddle_key.find('_mean')  > 0:
                tmp_key = torch_key.replace('running_mean','_mean')
                if tmp_key==paddle_key:
                    new_fan_weight_dict[paddle_key] = torch_weight[torch_key].detach().numpy()
                    print('_mean:', torch_key, ':::::::', paddle_key)

            elif torch_key.find('running_var') and paddle_key.find('_variance') > 0:
            ...
            # else:
            #     new_fan_weight_dict[paddle_key] = torch_weight[torch_key].detach().numpy()
    print(len(new_fan_weight_dict.keys()))
    print(new_fan_weight_dict.keys())
    #print(new_fan_weight_dict['backbone.bn1._mean'])
    model.set_dict(new_fan_weight_dict)
    fluid.dygraph.save_dygraph(model.state_dict(), './k400_tpn_r50f32s2')
